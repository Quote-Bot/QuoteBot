...
class Highlights(commands.Cog):
    ...
    async def on_message(self, msg):
        ...
        async with self.bot.db_connect() as db:
            ...
            for user_id, query in highlights:
                if user_id not in quoted and query in msg.content and user_id != msg.author.id:
    ...
    async def highlight(self, ctx, *, query: str):
        ...
        async with self.bot.db_connect() as db:
            ...
            await db.execute("INSERT OR IGNORE INTO highlight VALUES (?, ?)", (user_id, query := query.lower().replace('\n', ' ')))
            await db.commit()
        await ctx.send(f"{self.bot.config['response_strings']['success']} {(await self.bot.localize(guild, 'HIGHLIGHTS_highlight_added')).format(escape_backticks_in_code(query))}")
    ...
    async def highlightremove(self, ctx, *, query: str):
        guild = getattr(ctx, 'guild', None)
        async with self.bot.db_connect() as db:
            async with db.execute("SELECT * FROM highlight WHERE user_id = ? AND query = ?", (user_id := ctx.author.id, query := query.lower().replace('\n', ' '))) as cur:
                if await cur.fetchone():
                    await db.execute("DELETE FROM highlight WHERE user_id = ? AND query = ?", (user_id, query))
                else:
                    async with db.execute("SELECT query FROM highlight WHERE user_id = ? AND query LIKE ?", (user_id, f'{query}%')) as cur:
                ...
                await db.commit()
        await ctx.send(f"{self.bot.config['response_strings']['success']} {(await self.bot.localize(guild, 'HIGHLIGHTS_highlightremove_removed')).format(escape_backticks_in_code(query))}")
